...
def patient_account():
    ...
    if patient_email:
        # Fetch patient details from the database based on the email
        patient_details = PatientUser.find_one({'email': patient_email})
...
def bloodbank_dispatched_orders():
    ...
    if date_from and date_to:
        try:
            ...
            filter_conditions['$expr'] = {
                '$and': [
                    {'$gte': [{'$substr': ['$timeofdispatch', 0, 10]}, date_from]},
                    {'$lte': [{'$substr': ['$timeofdispatch', 0, 10]}, date_to]}
                ]
            }
        except ValueError:
            # Handle invalid date format by setting the filter to current day
            date_from = date_to = datetime.today().strftime("%Y-%m-%d")
            filter_conditions['$expr'] = {
                '$eq': [{'$substr': ['$timeofdispatch', 0, 10]}, date_from]
            }
    else:
        # Set the filter for the current day if no date filters are provided
        today_str = datetime.today().strftime("%Y-%m-%d")
    ...
    return render_template('BBDispatch.html', orders=order_list, sort_order=sort_order, date_from=date_from, date_to=date_to)
...
def Blood_bag_inProgress():
    ...
    try:
        ...
        orders = Order.find({'BloodBank_Id': bb_reg_no, 'status': 'undelivered'}).sort('timestamp', sort_direction)
...
def Hosp_Blood_bag_inProgress():
    ...
    # Set default date range to today if no date filters are provided
